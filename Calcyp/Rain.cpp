...
Rain::Rain(bool bCreateRain, int _NumberOfYears)
{
	srand(time(NULL));
	...
	io::CSVReader<2> inInterval("./../DB/intervalCDF.csv");
	io::CSVReader<2> inAmount("./../DB/AmountCDF.csv");

	//in.read_header(io::ignore_no_column, "Prob", "Interval");
	float Prob, value;
	int i = 0;
	while (inInterval.read_row<float,float>(Prob, value)) {
		Interval.push_back(make_pair(Prob, value));
	}

	while (inAmount.read_row<float, float>(Prob, value)) {
		Amount.push_back(make_pair(Prob, value));
	}
	...
	//	sizerain[i] = (j - 0.05) + (0.05*((double)rand() / RAND_MAX));
